// app/dashboard/cases/datatable.tsx
"use client"
...
function AddDocumentLinkDialog({ caseId, caseName }: { caseId: string; caseName: string }) {
    ...
    const handleAddLink = async (values: z.infer<typeof addDocumentFormSchema>) => {
        ...
        try {
          ...
          toast({
              ...
              description: `A link to "${values.fileName}" has been added to the case.`,
          });
          form.reset();
          setOpen(false);
        } catch (error) {
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Add a link to a document for case: <span className="font-semibold">{caseName}</span>
                    </DialogDescription>
                </DialogHeader>
                <Form {...form}>
...
const columns: ColumnDef<Case>[] = [
  {
    ...
    header: "Case Title",
    cell: ({ row }) => {
        const singleCase = row.original;
        return (
             <Link href={`/dashboard/cases/${singleCase.id}`} className="font-medium text-primary hover:underline">
                {singleCase.caseName}
            </Link>
        )
    }
      ...
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem
              onClick={() => {
                navigator.clipboard.writeText(singleCase.id)
                toast({
            ...
            <DropdownMenuItem asChild>
              <Link href={`/dashboard/cases/${singleCase.id}`}>
                <FileText className="mr-2 h-4 w-4" />
                View case details
              </Link>
            </DropdownMenuItem>
            ...
            <UploadDocumentDialog caseId={singleCase.id} caseName={singleCase.caseName} />

            {/* Add link fallback */}
            <AddDocumentLinkDialog caseId={singleCase.id} caseName={singleCase.caseName} />
          </DropdownMenuContent>
        </DropdownMenu>
      )
    },
...
export function CaseDataTable({ data }: { data: Case[] }) {
    ...
    <div className="w-full">
      <div className="flex items-center py-4">
        <Input
          ...
          onChange={(event) =>
            table.getColumn("caseName")?.setFilterValue(event.target.value)
          }
          className="max-w-sm"
        />
        <CreateCaseForm />
