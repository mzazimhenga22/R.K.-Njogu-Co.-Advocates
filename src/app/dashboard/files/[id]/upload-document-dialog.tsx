...
export default function UploadDocumentDialog({ fileId, fileName }: Props) {
  ...
  async function handleScanAndSave() {
    ...
    try {
      ...
      toast({
        title: "Scan saved",
        description: `${file.name} scanned and extracted text saved to file.`,
      });
    ...
    <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { setFile(null); setExtractedPreview(null); setProgressText(null);} }}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Select a PDF or image to scan. The app will extract text and save the scanned data to the file (no file upload).
          </DialogDescription>
        </DialogHeader>
